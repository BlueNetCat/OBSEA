...
export const OceanVertShader = /* glsl */ `
  ...
  void main() {
    ...
    // Gerstner Waves
    modPos += GerstnerWave(u_wave1Params, position, tangent, binormal);  // should be position instead of modPos inside function, but wave loops appear
    // Attenuation
    ...
    modPos += GerstnerWave(u_wave2Params, position, tangent, binormal); // should be position instead of modPos inside function, but wave loops appear
    // Attenuation
    modPos.y *= distanceFactor;
    tangent.x /= distanceFactor;
    binormal.z /= distanceFactor;
    ...
    modPos += GerstnerWave(u_wave3Params, position, tangent, binormal); // should be position instead of modPos inside function, but wave loops appear
    // Attenuation
    modPos.y *= distanceFactor;
    tangent.x /= distanceFactor;
    binormal.z /= distanceFactor;
    ...
    for (int i = 0; i < int(u_imgSize.x); i++){
      for (int j = 0; j < int(u_imgSize.y); j++){
        ...
        modPos += GerstnerWave(params, position, tangent, binormal); // should be position instead of modPos inside function, but wave loops appear
        // Attenuation
        modPos.y *= distanceFactor;
        tangent.x /= distanceFactor;
        binormal.z /= distanceFactor;
